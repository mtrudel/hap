defmodule HAP.PairSetup do
  ...
  def handle_call(%{@kTLVType_State => <<1>>, @kTLVType_Method => <<0>>}, _from, %{step: 1} = state) do
    if HAP.AccessoryServerManager.paired?() do
      ...
      p = HAP.AccessoryServerManager.pairing_code()
      {:ok, s, v} = HAP.Crypto.SRP6A.verifier(@i, p)
      {:ok, auth_context, b} = HAP.Crypto.SRP6A.auth_context(v)
  ...
  def handle_call(
    ...
    {:ok, m_1, m_2, k} = HAP.Crypto.SRP6A.shared_key(auth_context, a, @i, s)
  ...
  def handle_call(
    ...
    with {:ok, envelope_key} <-
           HAP.Crypto.HKDF.generate(session_key, "Pair-Setup-Encrypt-Salt", "Pair-Setup-Encrypt-Info"),
         {:ok, tlv} <- HAP.Crypto.ChaCha20.decrypt_and_verify(encrypted_data, envelope_key, "PS-Msg05"),
         {:ok, ios_identifier, ios_ltpk} <- extract_ios_device_exchange(tlv, session_key),
         {:ok, response_sub_tlv} <-
           build_accessory_device_exchange(
             HAP.AccessoryServerManager.identifier(),
             HAP.AccessoryServerManager.ltpk(),
             HAP.AccessoryServerManager.ltsk(),
             session_key
           ),
         {:ok, encrypted_response} <- HAP.Crypto.ChaCha20.encrypt_and_tag(response_sub_tlv, envelope_key, "PS-Msg06") do
      HAP.AccessoryServerManager.add_controller_pairing(ios_identifier, ios_ltpk, @kFlag_Admin)
  ...
  defp extract_ios_device_exchange(tlv, session_key) do
    ...
    |> case do
      %{
        ...
        case HAP.Crypto.EDDSA.verify(ios_device_info, ios_signature, ios_ltpk) do
  ...
  defp build_accessory_device_exchange(accessory_identifier, accessory_ltpk, accessory_ltsk, session_key) do
    ...
    {:ok, accessory_signature} = HAP.Crypto.EDDSA.sign(accessory_info, accessory_ltsk)
